...
export async function inviteClientUser(
  ...
  if (existingProfile) {
    ...
    if (existingLink) {
      ...
      if (resendError) {
        ...
        // envia link de redefinição via /auth/callback (client-side, lê o hash fragment)
    ...
    // Reenviar convite — se falhar (usuário já confirmado), usa reset de senha como fallback
  ...
  if (inviteError || !inviteData?.user) {
    ...
    // SMTP padrão falhando (rate limit ou indisponível) — orientar a configurar SMTP customizado
    if (
      errCode === "unexpected_failure" ||
      errMsg.toLowerCase().includes("rate") ||
      errMsg.toLowerCase().includes("sending")
    ) {
      return { success: false, error: "Falha ao enviar e-mail. O SMTP padrão do Supabase atingiu o limite — configure o Resend em Authentication → SMTP Settings." };
    }
  ...
  if (!newProfile) {
    ...
    if (profileError || !manualProfile) {
      ...
      return { success: false, error: "Convite enviado, mas erro ao criar perfil. Contate o suporte." };
    }
    ...
    if (linkError) {
      ...
      return { success: false, error: "Convite enviado, mas erro ao vincular cliente." };
    }
  ...
  if (linkError) {
    ...
    return { success: false, error: "Convite enviado, mas erro ao vincular ao cliente." };
  }
...
export async function removeClientAccess(
  ...
  const staff = await requireStaff();
  if (!staff) return { success: false, error: "Sem permissão" };
