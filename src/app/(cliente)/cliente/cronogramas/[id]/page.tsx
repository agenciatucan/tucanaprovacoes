...
export default async function CronogramaPage({ params }: Props) {
  ...
  const { data: campaign } = await supabase
    .from('campaigns')
    .select('id, name, status, period_label, overview, start_date, end_date, client_id, is_locked, clients(name, company_name)')
    .eq('id', id).single();
  ...
  const startLabel = campaign.start_date
    ? new Date(campaign.start_date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }).replace('.', '')
    : null;
  const endLabel = campaign.end_date
    ? new Date(campaign.end_date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }).replace('.', '')
    : null;
  ...
  return (
    <div className="page">
      ...
      {/* Campaign hero header — green card from design */}
      <div className="card-lg" style={{
        ...
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 24 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 8 }}>
              {client?.company_name ?? client?.name}
            </div>
            <h1 style={{ margin: '0 0 12px', fontSize: 28, fontWeight: 700, letterSpacing: '-0.025em', lineHeight: 1.1, color: '#fff' }}>
              {campaign.name}
            </h1>
      ...
      {/* Week tabs + posts — client component handles interactivity */}
      {weekKeys.length > 0 ? (
        <CronogramaWeekTabs
          weekKeys={weekKeys}
          postsByWeek={weeks as Record<string, any[]>}
          campaignId={id}
        />
      ) : (
